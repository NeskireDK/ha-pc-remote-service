...
- Downloads SoundVolumeView for audio control
...
- Config is stored in `%AppData%\HaPcRemote\`
...
One free NirSoft tool is required for audio features. The installer downloads it automatically. For portable installs, place it in the `ToolsPath` directory (default: `./tools`).
...
See [PLANNED_FEATURES.md](PLANNED_FEATURES.md) for bugs, planned features, and version roadmap covering both repositories.
