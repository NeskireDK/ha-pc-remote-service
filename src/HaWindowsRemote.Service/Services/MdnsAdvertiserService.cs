...
/// _windows-remote._tcp.local. with SRV, TXT, and A records.
...
public sealed class MdnsAdvertiserService : IHostedService, IDisposable
{
    ...
    private const string ServiceType = "_windows-remote._tcp.local.";
    ...
    public MdnsAdvertiserService(IOptions<PcRemoteOptions> options, ILogger<MdnsAdvertiserService> logger)
    {
        ...
        _instanceName = $"{Environment.MachineName}._windows-remote._tcp.local.";
    ...
    private byte[] BuildResponsePacket()
    {
        ...
        // PTR record: _windows-remote._tcp.local. -> instance (shared record, no cache-flush)
