...
public sealed partial class LinuxMonitorService(ICliRunner cliRunner, ILogger<LinuxMonitorService> logger) : IMonitorService
{
    ...
    {
        // xrandr does not support named profiles — return empty list
        logger.LogDebug("Monitor profiles are not supported on Linux");
        return Task.FromResult(new List<MonitorProfile>());
    }

    public Task ApplyProfileAsync(string profileName)
    {
        throw new NotSupportedException("Monitor profiles are not supported on Linux.");
    }
